...
{
    setTitle("功能模块");
    QVBoxLayout *mainLayout = new QVBoxLayout(getMainWnd());  // 注意这里直接设置给this
    ...
    QString buttonStyle =
        ...
        "   border: 2px solid #0E0E91;"     // 边框
        "   background-color: #0E0E91;"     // 深蓝色背景
        "}"
        "QPushButton:hover {"
        "   background-color: #3D3DF0;"     // 悬停时稍亮的蓝色
        "}"
        "QPushButton:pressed {"
        "   background-color: #090950;"     // 按下时更深的蓝色
        "}"
        "QPushButton { font-size: 18pt; }";

    // 创建按钮
    QPushButton *buttons[] = {
        new QPushButton("导航", this),
        new QPushButton("音乐", this),
        new QPushButton("天气", this),
        // new QPushButton("天气", this),
        new QPushButton("设置", this)
    };
    ...
    for (QPushButton *btn : buttons) {
        ...
        // 强制正方形
        btn->setMinimumSize(80, 80); // 最小尺寸
    }

    // 将按钮添加到布局
    for (QPushButton *btn : buttons) {
        buttonLayout->addWidget(btn, 1); // 拉伸因子为1
    }

    // 将按钮布局添加到主布局
    mainLayout->addLayout(buttonLayout, 1); // 拉伸因子为1
